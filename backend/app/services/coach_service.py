...
class CoachService:
    ...
    def calculate_hr_zones(
        self, max_hr: int, resting_hr: int = 60, ftp_watts: Optional[int] = None
        ...
        """
        logger.info(
            f"📊 Calculating HR zones: max_hr={max_hr}, resting_hr={resting_hr}, ftp_watts={ftp_watts}"
        )
        ...
        logger.info(
            f"✅ HR zones calculated successfully - 5 zones with Karvonen formula"
        )
    ...
    def identify_workout_zone(
        self, avg_hr: int, max_hr: int, resting_hr: int = 60
    ) -> str:
    ...
    def build_athlete_context(
        ...
        if user.max_heart_rate:
            ...
            for zone_key, zone_info in zones.items():
                context_parts.append(
                    f"  {zone_info['name']}: {zone_info['min_bpm']}-{zone_info['max_bpm']} bpm"
                )
        ...
        if goals and len(goals) > 0:
            ...
            for goal in goals:
                if not goal.get("completed"):
                    ...
                    context_parts.append(
                        f"- {goal['name']} (Objetivo: {target}, Fecha: {deadline})"
                    )
        ...
        if recent_workouts:
            ...
            avg_pace = (
                sum(w.avg_pace for w in recent_workouts if w.avg_pace)
                / len([w for w in recent_workouts if w.avg_pace])
                if any(w.avg_pace for w in recent_workouts)
                else 0
            )
            ...
            for i, workout in enumerate(recent_workouts[:3], 1):
                ...
                hr_str = (
                    f"{workout.avg_heart_rate} bpm" if workout.avg_heart_rate else "N/A"
                )
                context_parts.append(
                    f"  {i}. {date_str}: {distance:.1f}km, pace {pace_str}, FC {hr_str}"
                )
        ...
        if user.injuries and len(user.injuries) > 0:
            ...
            if active_injuries:
                ...
                for injury in active_injuries:
                    context_parts.append(
                        f"- {injury.get('injury_type')}: {injury.get('description', 'N/A')}"
                    )
        ...
        if user.preferences:
            ...
            if user.preferences.get("time_of_day"):
                context_parts.append(
                    f"- Horario preferido: {user.preferences['time_of_day']}"
                )
            if user.preferences.get("terrain_preference"):
                context_parts.append(
                    f"- Terreno preferido: {user.preferences['terrain_preference']}"
                )
    ...
    def get_coaching_style_prompt(
        self, style: str, custom_prompt: Optional[str] = None
    ) -> str:
...
Sé conciso pero útil. Máximo 200 palabras."""
        ...
        try:
            ...
            return {
                ...
                "workout_summary": {
                    ...
                    "avg_pace": (
                        self._format_pace(workout.avg_pace)
                        if workout.avg_pace
                        else None
                    ),
                    "avg_hr": workout.avg_heart_rate,
                    "zone": (
                        self.identify_workout_zone(
                            workout.avg_heart_rate, user.max_heart_rate
                        )
                        if workout.avg_heart_rate and user.max_heart_rate
                        else None
                    ),
                },
            }
    ...
    def _format_workout_details(
        self, workout: models.Workout, user: models.User
    ) -> str:
        ...
        details.append(
            f"Duración: {workout.duration_seconds // 60} min {workout.duration_seconds % 60} seg"
        )
        ...
        if workout.avg_heart_rate:
            ...
            if user.max_heart_rate:
                zone = self.identify_workout_zone(
                    workout.avg_heart_rate, user.max_heart_rate
                )
                zones = self.calculate_hr_zones(user.max_heart_rate)
                ...
                if zone_info:
                    details.append(
                        f"Zona cardíaca: {zone_info['name']} ({zone_info['percentage']})"
                    )
    ...
    def analyze_running_form(
        ...
        if workout.avg_pace:
            ...
            if workout.max_speed and workout.distance_meters > 0:
                avg_speed = (workout.distance_meters / 1000) / (
                ...
                if speed_variance > 0.3:
                    issues.append(
                        "Gran variación en velocidad durante el entrenamiento"
                    )
                    recommendations.append(
                        "Trabaja en mantener un ritmo más constante, especialmente en entrenamientos de base"
                    )
        ...
        if workout.avg_heart_rate and workout.max_heart_rate:
            ...
            if hr_variance > 30:
                ...
                recommendations.append(
                    "Considera hacer warm-up más largo y mantener ritmo más controlado"
                )
        ...
        if workout.elevation_gain and workout.distance_meters:
            ...
            if elevation_per_km > 50:
                recommendations.append(
                    "Terreno con mucho desnivel: enfócate en pasos cortos y mantén cadencia alta en subidas"
                )
        ...
        if workout.duration_seconds and workout.distance_meters:
            ...
            form_metrics["efficiency"] = (
                "good"
                if pace_seconds < 360
                else "moderate" if pace_seconds < 420 else "low"
            )

        # Build form analysis with AI
        form_summary = self._generate_form_analysis_ai(
            form_metrics, issues, recommendations, user
        )
...
Mantén respuestas concisas (máximo 200 palabras) a menos que se solicite más detalle."""
    ...
    def analyze_workout_deep(
        self, db: Session, user: models.User, workout: models.Workout
    ) -> Dict[str, Any]:
        """
        ...
        if user.goals:
            ...
            for goal in user.goals[:3]:
                goals_context += (
                    f"- {goal.get('target', 'N/A')} ({goal.get('type', 'N/A')})\n"
                )
...
FC máxima: {workout.max_heart_rate or 'N/A'} bpm"""
    ...
    def calculate_readiness_score(
        self, health_metric: Optional[models.HealthMetric], user: models.User
    ) -> Dict[str, Any]:
        """
            ...
            factors.append(
                {
                    ...
                    "weight": weight,
                    "status": (
                        "good" if score >= 70 else "moderate" if score >= 50 else "low"
                    ),
                }
            )
            ...
            factors.append(
                {
                    ...
                    "weight": weight,
                    "status": (
                        "good" if score >= 70 else "moderate" if score >= 50 else "poor"
                    ),
                }
            )
            weighted_sum += score * weight
            ...
            factors.append(
                {
                    ...
                    "weight": weight,
                    "status": (
                        "good" if hours >= 7 else "moderate" if hours >= 6 else "poor"
                    ),
                }
            )
            weighted_sum += score * weight
        ...
        if health_metric.resting_hr_bpm and health_metric.resting_hr_baseline_bpm:
            ...
            if diff <= 0:
                ...
                score = max(
                    0, 100 - (diff * 10)
                )  # Each bpm above baseline = -10 points
            weight = 0.1
            ...
            factors.append(
                {
                    ...
                    "status": (
                        "low"
                        if health_metric.stress_level < 30
                        else "moderate" if health_metric.stress_level < 60 else "high"
                    ),
                }
            )
            weighted_sum += score * weight
            ...
            factors.append(
                {
                    ...
                    "status": (
                        "high"
                        if health_metric.energy_level >= 4
                        else "moderate" if health_metric.energy_level >= 3 else "low"
                    ),
                }
            )
            weighted_sum += score * weight
        ...
        if total_weight == 0:
            ...
            confidence = (
                "high"
                if total_weight >= 0.6
                else "medium" if total_weight >= 0.3 else "low"
            )
        ...
        if readiness_score >= 75:
            ...
            recommendation = (
                "⚠️ Estado moderado. Entrenamientos ligeros o moderados recomendados."
            )
    ...
    def generate_health_aware_recommendation(
        ...
        recent_workouts = (
            db.query(models.Workout)
            .filter(
                models.Workout.user_id == user.id, models.Workout.start_time >= week_ago
            )
            .order_by(models.Workout.start_time.desc())
            .all()
        )
...
"""
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": self.get_coaching_style_prompt(
                            user.coaching_style or "balanced"
                        ),
                    },
                    {"role": "user", "content": prompt},
                ],
        ...
        return {
            ...
            "health_metrics": (
                self._format_health_metrics(health_metric) if health_metric else None
            ),
        }

    def _build_health_context(
        self, health_metric: Optional[models.HealthMetric], readiness: Dict[str, Any]
    ) -> str:
        ...
        lines.append(
            f"- Readiness Score: {readiness['readiness_score']}/100 ({readiness['confidence']} confidence)"
        )

        for factor in readiness["factors"]:
            detail = factor.get("detail", "")
            lines.append(
                f"- {factor['name']}: {factor['score']}/100 ({factor['status']}) {detail}"
            )
    ...
    def _format_health_metrics(
        self, health_metric: models.HealthMetric
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "sleep_duration_hours": (
                health_metric.sleep_duration_minutes / 60
                if health_metric.sleep_duration_minutes
                else None
            ),
    ...
    def generate_personalized_training_plan(
        ...
        end_date = start_date + timedelta(
            weeks=plan_request.plan_duration_weeks - 1, days=6
        )
...
"""
        ...
        method_label = (
            "Ritmo (min/km)"
            if training_method == "pace_based"
            else "Frecuencia Cardíaca (Zonas HR)"
        )
        method_context = f"""
...
"""
        ...
        recent_weeks_with_data = (
            min(
                len(set(w.start_time.isocalendar()[1] for w in recent_workouts[-20:])),
                10,
            )
            or 1
        )
        avg_weekly_km = (
            recent_distance_km / recent_weeks_with_data
            if recent_weeks_with_data > 0
            else 20
        )
...
Retorna SOLO el JSON válido, sin explicaciones adicionales."""
        ...
        try:
            ...
            return self._generate_fallback_plan(
                plan_request, start_date, end_date, training_method
            )
    ...
    def _generate_fallback_plan(
        ...
        base_distance = (
            40 if plan_request.general_goal in ["marathon", "build_endurance"] else 25
        )
        ...
        for week_num in range(1, plan_request.plan_duration_weeks + 1):
            ...
            if plan_request.training_days_per_week >= 4:
                training_days.append(
                    ("Long Run", "Tirada larga", 12 * week_progression, 6.8)
                )
            if plan_request.training_days_per_week >= 5:
                training_days.append(
                    ("Recovery", "Carrera de recuperación", 5 * week_progression, 7.0)
                )

            for day_idx, (day_type, description, distance, pace) in enumerate(
                training_days[: plan_request.training_days_per_week]
            ):
                ...
                days.append(
                    {
                        ...
                        "heart_rate_zone": (
                            2
                            if "Easy" in day_type or "Recovery" in day_type
                            else 3 if "Tempo" in day_type else 4
                        ),
            ...
            weeks.append(
                {"week_number": week_num, "days": days, "total_km": round(total_km, 1)}
            )
