...
class User(Base):
    ...
    resting_heart_rate = Column(Integer, nullable=True)  # Resting HR in bpm
    vo2_max = Column(Float, nullable=True)  # VO2 Max in ml/kg/min
    ...
    # Athlete Profile fields
    running_level = Column(
        String, nullable=True, default="intermediate"
    )  # beginner/intermediate/advanced
    max_heart_rate = Column(Integer, nullable=True)  # bpm
    goals = Column(
        JSON, nullable=True, default=list
    )  # [{"name": "sub-40 10K", "target_pace": "4:00", "deadline": "2025-12-31", "type": "race"}]
    coaching_style = Column(
        String, nullable=True, default="balanced"
    )  # motivator/technical/balanced/custom
    injuries = Column(
        JSON, nullable=True, default=list
    )  # [{"date": "2024-06-15", "type": "knee", "description": "IT band", "recovered": true}]
    preferences = Column(
        JSON, nullable=True, default=dict
    )  # {"music": true, "preferred_pace_range": [5, 6], "time_of_day": "evening"}
    ...
    primary_device = Column(
        String, nullable=True
    )  # garmin, xiaomi, strava, manual, apple
    use_case = Column(
        String, nullable=True
    )  # fitness_tracker, training_coach, race_prep, general_health
    coach_style_preference = Column(
        String, nullable=True
    )  # motivator, technical, balanced, custom
    language = Column(String, default="es", nullable=False)  # es, en, fr, etc
    enable_notifications = Column(Boolean, default=True, nullable=False)
    integration_sources = Column(
        JSON, nullable=True, default=list
    )  # ["garmin", "strava", "google_fit"] - order matters
    onboarding_completed_at = Column(DateTime, nullable=True)

    # Multi-device configuration
    devices_configured = Column(
        JSON, nullable=True, default=list
    )  # List of configured device IDs
    device_sync_config = Column(
        JSON, nullable=True, default=dict
    )  # {"garmin": {"enabled": true, "sync_interval_hours": 1, "last_sync": "2025-11-14T10:30:00", "auto_sync": true}, ...}
    device_sync_enabled = Column(
        Boolean, default=True, nullable=False
    )  # Master toggle for all syncing
...
class Workout(Base):
    ...
    avg_stride_length = Column(Float, nullable=True)  # meters
    ...
    source_type = Column(
        String, nullable=True, default="garmin_fit"
    )  # garmin_fit, garmin_oauth, gpx_upload, tcx_upload, strava
    data_quality = Column(
        String, nullable=True, default="high"
    )  # high (FIT), medium (GPX with HR), basic (GPX minimal)
