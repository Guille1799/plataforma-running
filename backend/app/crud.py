...
def create_workout(db: Session, user_id: int, workout_data: schemas.WorkoutCreate) -> models.Workout:
...
def get_user_workout_stats(db: Session, user_id: int) -> schemas.WorkoutStats:
    ...
    avg_heart_rate = sum(w.avg_heart_rate for w in workouts if w.avg_heart_rate) / len([w for w in workouts if w.avg_heart_rate]) if any(w.avg_heart_rate for w in workouts) else None
    total_calories = sum(w.calories for w in workouts if w.calories)
    
    # Calculate average pace
    total_distance_km_for_pace = sum(w.distance_meters for w in workouts if w.avg_pace) / 1000
    avg_pace_sec_per_km = sum(w.duration_seconds for w in workouts if w.avg_pace) / total_distance_km_for_pace if total_distance_km_for_pace > 0 else None
    
    # Sports breakdown
    sports_breakdown: dict = {}
    for workout in workouts:
        sports_breakdown[workout.sport_type] = sports_breakdown.get(workout.sport_type, 0) + 1
    
    return schemas.WorkoutStats(
        total_workouts=len(workouts),
        total_distance_km=round(total_distance_km, 2),
        total_duration_hours=round(total_duration_hours, 2),
        avg_pace_min_per_km=round(avg_pace_sec_per_km / 60, 2) if avg_pace_sec_per_km else None,
