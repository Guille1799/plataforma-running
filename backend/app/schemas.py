...
class WorkoutCreate(BaseModel):
    ...
    avg_stride_length: Optional[float] = Field(None, ge=0)  # meters
...
class Goal(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=200, description="Nombre del objetivo"
    )
    goal_type: GoalType
...
class AthletePreferences(BaseModel):
    ...
    custom_prompt: Optional[str] = Field(
        None,
        max_length=1000,
        description="Prompt personalizado para coaching_style=custom",
    )
...
class AthleteProfileOut(BaseModel):
    ...
    # Physical attributes (from Garmin sync)
    height_cm: Optional[float] = None
    weight_kg: Optional[float] = None

    # Physiological metrics (from Garmin sync)
...
class ChatMessageCreate(BaseModel):
    ...
    message: str = Field(
        ..., min_length=1, max_length=2000, description="User message to coach"
    )
...
class OnboardingCompleteRequest(BaseModel):
    ...
    integration_sources: List[str] = Field(
        default=[], description="Additional integration sources in priority order"
    )
...
class DeviceSyncConfig(BaseModel):
    ...
    sync_interval_hours: int = Field(
        ge=1, le=24, description="Sync interval in hours (1-24)"
    )
...
class DeviceIntegration(BaseModel):
    ...
    device_id: str = Field(
        description="Unique device identifier (e.g., 'garmin', 'xiaomi', 'strava')"
    )
    device_type: str = Field(
        description="Type of device (garmin, xiaomi, strava, apple, manual)"
    )
...
class DeviceIntegrationCreate(BaseModel):
    ...
    device_type: str = Field(
        description="Device type (garmin, xiaomi, strava, apple, manual)"
    )
...
class TrainingPlanRequest(BaseModel):
    ...
    general_goal: str = Field(
        ...,
        description="marathon, half_marathon, 10k, 5k, improve_fitness, build_endurance",
    )
    ...
    preferred_long_run_day: str = Field(
        default="saturday", description="day of week for long run"
    )
    ...
    training_method: str = Field(
        default="automatic", description="automatic, pace_based or heart_rate_based"
    )
    include_cross_training: bool = False
    cross_training_types: List[str] = Field(
        default=[], description="swimming, cycling, rowing, etc"
    )
    recovery_focus: str = Field(
        default="moderate", description="minimal, moderate, high"
    )
